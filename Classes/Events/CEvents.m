    ...
    else
    {
        ...
        *pY=[pHo getY];
        ...
        if (pPos->posFlags&CPF_ACTION)                    // Relatif au point d'action?
        {
            ...
            {
                if ( pHo->roc->rcImage != (unsigned short)-1 )
                {
...
void actSetFrameEffect(event*pe, CRun* rhPtr)    // ACT_SETFRAMEEFFECT
{
    ...
    // Get effect index
    if (pEvp->evpCode == 11) {  // PARAM_SHORT
        effect = pEvp->evp.evpW.evpW0;
    } else { //PARAM_STRING
        char* pName=(char*)&pEvp->evp.evpW.evpW0;
        NSString* effectName;
        if(rhPtr->rhApp->bUnicode)
            effectName = [[NSString alloc] initWithCharacters:(unichar*)pName length:strUnicharLen((unichar*)pName)];
        else
            effectName = [[NSString alloc] initWithCString:pName encoding:NSWindowsCP1252StringEncoding];

        if (effectName != nil && ![effectName isEqualToString:@""]) {
            effect = [rhPtr->rhApp->effectBank getUniversalEffectIndex:effectName];
        }
        [effectName release];
    }

    // Create shader if it's not a standard effect
    if (effect >= EFFECT_SHADERBASEINDEX) {
        if ([frame checkOrCreateEffectIfNeededByIndex:(effect - EFFECT_SHADERBASEINDEX)] != -1) {
            effect = BOP_EFFECTEX;
            frame->effectShader = [frame->effectEx getIndexShader];
            frame->hasFrameEffect = YES;
        } else {
            effect = BOP_COPY;
            frame->hasFrameEffect = NO;
        }
    }
...
void actSetFrameAlphaCoef(event*pe, CRun* rhPtr)    // ACT_SETFRAMEALPHACOEF
{
    ...
    if((frame->effect & BOP_RGBAFILTER) != 0)
    ...
    if ((frame->effect & BOP_MASK) == BOP_EFFECTEX)
    {
        if(frame->effectEx != nil)
        {
            [frame->effectEx setRGBA:(alphaPart | rgbPart)];
        }
    }
    else if ((frame->effect & BOP_MASK) == BOP_BLEND)
        frame->effect = BOP_COPY | BOP_RGBAFILTER;
    else
...
void actSetFrameRGBCoef(event*pe, CRun* rhPtr)    // ACT_SETFRAMERGBCOEF
{
    ...
    unsigned int effect = frame->effect;
    unsigned int effectParam = frame->effectParam;

    unsigned int rgbPart = swapRGB((argb & 0x00FFFFFF));
    unsigned int alphaPart = 0xFF000000;
    if((effect & BOP_RGBAFILTER) != 0)
    {
        alphaPart = (effectParam & 0xFF000000);
    }
    else if((effect & BOP_MASK) == BOP_EFFECTEX)
    {
        if(frame->effectEx != nil)
        {
            alphaPart = [frame->effectEx getRGBA] & 0xFF000000;
            [frame->effectEx setRGBA:(alphaPart | rgbPart)];
        }
    }
    else
...
void actPla_FinishLives(CRun* rhPtr, int joueur, int live)
{
    ...
    if (live==0)
    {
        if (lives[joueur]!=0)
        {
            [rhPtr->rhEvtProg push_Event:0 withCode:CNDL_NOMORELIVE andParam:joueur andObject:nil andOI:(short)joueur];
        }
    }
...
{
    int joueur=pe->evtOi;
    if (rhPtr->rhEvtProg->rhCurParam[0] != joueur)
        return NO;
    int* lives = [rhPtr->rhApp getLives];
    int live=lives[joueur];
...
BOOL evaOnLoop(event* pe, CRun* rhPtr, LPHO pHo)
{
	...
	// Indexed loop
	if (pEvp->evpCode == PARAM_SHORT)
	{
		int fastLoopIndex = pEvp->evp.evpW.evpW0 - 1;
		if (fastLoopIndex >= 0 && fastLoopIndex < rhPtr->rhFrame->numberOfIndexedFastLoops)
		{
			if (rhPtr->rh4CurrentFastLoopIndex != fastLoopIndex)
				return NO;
	...
	else
	{
		...
		if (pToken->expCode.expSCode.expType==-1 && pToken->expCode.expSCode.expNum==3 && ((LPEXP)((LPBYTE)pToken+pToken->expSize))->expCode.expLCode.expCode==0)
			pName=(NSString*)[rhPtr->rhEvtProg->allocatedStrings get:(unsigned short)pToken->expu.expw.expWParam0];
		else
...
void actStartLoop(event* pe, CRun* rhPtr)
{
	...
	bool bIndexedFastLoops = (rhPtr->rhFrame->numberOfIndexedFastLoops != 0);
	NSString* pName = nil;
	int loopNumber = 0;

	if (bIndexedFastLoops)
	{
		loopNumber = pEvp->evp.evpW.evpW0;
		if (loopNumber <= 0 || loopNumber > rhPtr->rhFrame->numberOfIndexedFastLoops)
			return;
	}
    
	else if (rhPtr->rh4ComplexOnLoop == 0 && rhPtr->rh4PosOnLoop != nil)
	{
		loopNumber = pEvp->evp.evpW.evpW0;
		if (loopNumber == 0)
		{
	...
	CLoop* pLoop;
	if (bIndexedFastLoops)
		pLoop = (CLoop*)[rhPtr->rh4FastLoops get:(loopNumber - 1)];
	else
	{
		if (pName==nil)
			pName=[rhPtr get_EventExpressionString:pEvp];
		if (pName==nil)
			return;
		pLoop = [rhPtr addFastLoop:pName withIndexPtr:NULL];
	}
	if (pLoop == nil)
		return;
...
void actSetLoopIndex(event* pe, CRun* rhPtr)
{
	...
	// Indexed loop?
	if (rhPtr->rhFrame->numberOfIndexedFastLoops != 0)
	{
		int fastLoopIndex = pEvp->evp.evpW.evpW0 - 1;
		if (fastLoopIndex >= 0 && fastLoopIndex < rhPtr->rhFrame->numberOfIndexedFastLoops)
		{
...
void expSys_LoopIndex(CRun* rhPtr)
{
	...
	{
		int fastLoopIndex = pToken->expu.expl.expLParam - 1;
		nextToken();
		if (fastLoopIndex >= 0 && fastLoopIndex < rhPtr->rhFrame->numberOfIndexedFastLoops)
		{
			CLoop* pLoop = (CLoop*)[rhPtr->rh4FastLoops get:fastLoopIndex];
...
void actGrpActivate(event* pe, CRun* rhPtr)
{
	...
	LPEVG evgPtr = (*rhPtr->cndGroupAddress)[evpPtr->evp.evpW.evpW2];
    LPEVT evtPtr=EVGFIRSTEVT(evgPtr);
...
{
    LPEVP pEvp=(LPEVP)((LPBYTE)pe+CND_SIZE);
	LPEVG pEvg = (*rhPtr->cndGroupAddress)[pEvp->evp.evpW.evpW2];
...
BOOL evaStartChildEvent(event* pe, CRun* rhPtr, LPHO pHo)
{
    ...
    {
        const std::vector<saveSelection>& selectedObjects = rhPtr->rhChildEventSelectionStack.at(rhPtr->rhChildEventSelectionStack.size()-1);
...
*/
    getCurrentResult()->stringValue=(NSString*)[rhPtr->rhEvtProg->allocatedStrings get:(unsigned short)rhPtr->rh4ExpToken->expu.expw.expWParam0];
        ...
        while ((nOIL = *pShort) >= 0)
        {
            ...
            if (poil->oilEventCount == rhPtr->rhEvtProg->rh2EventCount)
            {
                ...
                while (num >= 0)
                {
                    ...
                    {
                        double distance = (double)(pHo2->hoX - pHo->hoX) * (double)(pHo2->hoX - pHo->hoX) + (double)(pHo2->hoY - pHo->hoY) * (double)(pHo2->hoY - pHo->hoY);
						if ([hoList size] == 0)
						{
            ...
            if ( ocPtr->ocAnimations->ahAnimExists[a] != 0 )
            {
                ...
                for(int d=0; d<32; ++d)
                {
                    ...
                    if(dir != nil)
                    {
                        ...
                        {
                            unsigned short handle = dir->adFrames[i];
...
void expSpr_GetRGBAt(CRun* rhPtr)
{
    ...
    int rgb = 0;
    unsigned short img = pHo->roc->rcImage;
    if ( img != (unsigned short)-1 )
    {
...
void actSetEffect(event* pe, CRun* rhPtr)
{
    ...
    // Create shader if it's not a standard effect
    if (effect >= EFFECT_SHADERBASEINDEX) {
        if ([pHo checkOrCreateEffectIfNeededByIndex:(effect - EFFECT_SHADERBASEINDEX) withEffectParam:pHo->ros->rsEffectParam andFillData:NO] != -1) {
            effect = BOP_EFFECTEX;
...
void actSetAlphaCoef(event* pe, CRun* rhPtr)
{
    ...
    if ((pHo->ros->rsEffect & BOP_MASK) == BOP_EFFECTEX)
    {
        if(pHo->hoEffect != nil)
        {
            [pHo->hoEffect setRGBA:(alphaPart | rgbPart)];
        }
    }
	else if ((pHo->ros->rsEffect & BOP_MASK) == BOP_BLEND)
...
void actSetRGBCoef(event* pe, CRun* rhPtr)
{
    ...
    unsigned int effect = pHo->ros->rsEffect;
    unsigned int effectParam = pHo->ros->rsEffectParam;
    ...
    else if((effect & BOP_MASK) == BOP_EFFECTEX)
    {
        if(pHo->hoEffect != nil)
        {
            alphaPart = [pHo->hoEffect getRGBA] & 0xFF000000;
            [pHo->hoEffect setRGBA:(alphaPart | rgbPart)];
        }
    }
    else
...
{
    nextToken();
    double dLow = [rhPtr get_ExpressionDouble];
    nextToken();
    double dHigh = [rhPtr get_ExpressionDouble];
    nextToken();
    double dRatio = [rhPtr get_ExpressionDouble];
    [getCurrentResult() forceDouble:(dLow * (1.0 - dRatio)) + (dHigh * dRatio)];
}
void expSys_invLerp(CRun* rhPtr)
{
    ...
    double dHigh = [rhPtr get_ExpressionDouble];
    nextToken();
    double dValue = [rhPtr get_ExpressionDouble];
    double ratio = (dValue - dLow) / (dHigh - dLow);
    [getCurrentResult() forceDouble:ratio];
}
    ...
    while(TRUE)
    {
        ...
        while(pForEach!=nil)
        {
            if (pForEach->index<0)
            {
                ...
                if (pForEach2 != nil)
                {
					if (pForEach2->named)
					{
						if (!pForEach->named || [pForEach->name caseInsensitiveCompare:pForEach2->name]!=0)
							pForEach2 = nil;
					}
					else
					{
						if (pForEach->nID != pForEach2->nID)
							pForEach2 = nil;
					}
                }
                break;
        ...
        if (!pForEach->named)
        {
            if (pForEach->number > 0)
            {
                ...
                nCondition = forEachLoopIDToConditionCode(pHo->hoType, pForEach->nID);
            }
        }
        if (pForEach2 != nil && !pForEach2->named)
        {
            if (pForEach2->number > 0)
            {
                LPHO pHo2 = pForEach2->objects[0];
                nCondition2 = forEachLoopIDToConditionCode(pHo2->hoType, pForEach2->nID);
            }
        }
...
BOOL RCND_EXTONLOOP(event* pe, CRun* rhPtr, CObject* pHo)
{
    ...
    {
        if ((!rhPtr->rh4CurrentForEach->named && rhPtr->rh4CurrentForEach->nID == nID) || (rhPtr->rh4CurrentForEach->named && [rhPtr->rh4CurrentForEach->name caseInsensitiveCompare:pName]==0))
        {
            if ( pMultipleParam )
            {
                if ( !CheckMultipleFlagAndVariableParam(pMultipleParam, pHo) )
    ...
    {
        if ((!rhPtr->rh4CurrentForEach2->named && rhPtr->rh4CurrentForEach2->nID == nID) || (rhPtr->rh4CurrentForEach2->named && [rhPtr->rh4CurrentForEach2->name caseInsensitiveCompare:pName]==0))
        {
            if ( pMultipleParam )
            {
                if ( !CheckMultipleFlagAndVariableParam(pMultipleParam, pHo) )
...
BOOL RCND_EXTONLOOP2(event* pe, CRun* rhPtr, CObject* pHo)
{
    ...
    if (pForEach!=nil)
    {
        if ((!pForEach->named && pForEach->nID == nID) || (pForEach->named && [pForEach->name caseInsensitiveCompare:pName]==0))
        {
...
void addForEach(NSString* pName, int nID, LPHO pHo, OINUM oil, CRun* rhPtr)
{
    ...
    while(pForEach!=nil)
    {
        if (pForEach->oi==oil && ((pForEach->named && [pName caseInsensitiveCompare:pForEach->name] == 0) || (!pForEach->named && pForEach->nID == nID)))
        {
...
void RACT_EXTFOREACH(event* pe, CRun* rhPtr)
{
    ...
    addForEach(pName, nID, pHo, pe->evtOi, rhPtr);
}
...
void RACT_EXTFOREACH2(event* pe, CRun* rhPtr)
{
    ...
    NSString* pName = nil;
	int nID = -1;
	if (pEvp2->evpCode == PARAM_SHORT)
		nID = pEvp2->evp.evpW.evpW0;
	else
	{
		LPEXP pToken = (LPEXP)&pEvp2->evp.evpW.evpW1;
		if (pToken->expCode.expSCode.expType==-1 && pToken->expCode.expSCode.expNum==3 && ((LPEXP)((LPBYTE)pToken+pToken->expSize))->expCode.expLCode.expCode==0)
			pName=(NSString*)[rhPtr->rhEvtProg->allocatedStrings get:(unsigned short)pToken->expu.expw.expWParam0];
		else
			pName=[rhPtr get_EventExpressionString:pEvp2];
	}
    addForEach(pName, nID, pHo, pe->evtOi, rhPtr);
    
    pHo=[rhPtr->rhEvtProg get_CurrentObjects:pEvp->evp.evpW.evpW0];
    if (pHo!=nil)
        addForEach(pName, nID, pHo, pEvp->evp.evpW.evpW1, rhPtr);
}
...
expSys_sign            ,    // 69EXP_SIGN
expSys_lerp                ,    // 70EXP_LERP
expSys_invLerp            ,    // 71EXP_INVLERP
};
