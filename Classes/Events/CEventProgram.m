...
-(void)load
{
    ...
    [self freeQualifierList];
    if (qualifiers!=nil)
    {
        for (int n=0; n<nQualifiers; n++)
        {
            [qualifiers[n] release];
        }
        free(qualifiers);
        qualifiers=nil;
    }
	while (true)
	{
        ...
        {
            short options = [app->file readAShort];
            if ( (options & EVENTOPTION_BREAKCHILD) != 0 )
                gEvgFlagsDefaultMask |= EVGFLAGS_BREAK;
            short nfl = [app->file readAShort];
			app->frame->numberOfIndexedFastLoops = nfl;
        }
		// EVTFILECHUNK_END
...
int forEachLoopIDToConditionCode(int type, int forEachID)
{
	...
	int cndIndex = CNDIDX_EXTONLOOP_INTERNAL_FIRST + forEachID;
	if (type == OBJ_SPR)
	{
		if (forEachID < CNDIDX_EXTONLOOP_INTERNAL_MAXNUMBER_ONLYACTIVEOBJECTS)
		{
			if (cndIndex > EVENTS_EXTBASE)
				cndIndex += NUMBER_OF_ACTIVEOBJECT_CONDITIONS;
			nCondition = -cndIndex * 65536;
		}
	}
	else
	{
		if (forEachID < CNDIDX_EXTONLOOP_INTERNAL_MAXNUMBER)
			nCondition = -cndIndex * 65536;
	}
	return nCondition;
}
...
-(short)assemblePrograms
{
	...
	do
	{
			...
			for (evtPtr=EVGFIRSTEVT(evgPtr), i=EVGNEVENTS(evgPtr); i>0; evtPtr=EVTNEXT(evtPtr), i--)
			{
                ...
                if ( evtPtr->evtCode.evtLCode.evtCode == ACTL_STARTLOOP )
                {
					...
					if (evpPtr->evpCode != PARAM_SHORT)
					{
						...
						{
							CPosStartLoop* pStartLoop = new CPosStartLoop(evpPtr, (NSString*)[allocatedStrings get:(unsigned short)pToken->expu.expw.expWParam0]);
                ...
                else if ( evtPtr->evtCode.evtLCode.evtCode == ACTL_STOPLOOP || evtPtr->evtCode.evtLCode.evtCode == ACTL_SETLOOPINDEX )
                {
					...
					if (evpPtr->evpCode != PARAM_SHORT)
					{
						...
						if ( pToken->expCode.expSCode.expType == -1 && pToken->expCode.expSCode.expNum == 3 && ((LPEXP)((LPBYTE)pToken+pToken->expSize))->expCode.expLCode.expCode == 0 )
						{
							NSString* pName = (NSString*)[allocatedStrings get:(unsigned short)pToken->expu.expw.expWParam0];
							int idx = -1;
				...
				else if ( evtPtr->evtCode.evtLCode.evtCode == CNDL_GROUP )
				{
                    evpPtr = EVTPARAMS(evtPtr);
					LPGRP grpPtr = (LPGRP)&evpPtr->evp.evpW.evpW0;
					if (grpPtr->grpId >= (int)rhPtr->cndGroupAddress->size())
						rhPtr->cndGroupAddress->resize(grpPtr->grpId+16, 0);
					rhPtr->cndGroupAddress->at(grpPtr->grpId) = evgPtr;
				}
			}
		...
		*colList=-1;
		...
		bool bOptimizeForEachActiveObjects = true;
		bool bOptimizeForEachOtherObjects = true;

		for (evgPtr=pEvents; evgPtr->evgSize!=0 && (bOptimizeForEachActiveObjects || bOptimizeForEachOtherObjects); evgPtr=EVGNEXT(evgPtr))
		{
			for (evtPtr=EVGFIRSTEVT(evgPtr), n = 0; n < evgPtr->evgNCond; evtPtr=EVTNEXT(evtPtr), n++)
			{
				...
				{
					// Loop defined by index?
					LPEVP evpPtr = EVTPARAMS(evtPtr);
					if (evpPtr->evpCode != PARAM_SHORT)
					{
						if (type == OBJ_SPR)
							bOptimizeForEachActiveObjects = false;
						else
							bOptimizeForEachOtherObjects = false;
					}
				}
			}
					
			for (n = 0; n < evgPtr->evgNAct && (bOptimizeForEachActiveObjects || bOptimizeForEachOtherObjects); evtPtr = EVTNEXT(evtPtr), n++)
			{
				type = EVTTYPE(evtPtr->evtCode.evtLCode.evtCode);
        ...
        for (oil = 0; oil < rhPtr->rhMaxOI; oil++, uilPtr++)
        {
			*uilPtr=ss;
            if (rhPtr->rhOiList[oil]->oilType < KPX_BASE)
            {
                ...
                ss += MAX(nConditions[NUMBEROF_SYSTEMTYPES + rhPtr->rhOiList[oil]->oilType] + 1, EVENTS_EXTBASE + 1);
            }
            else
            {
		...
		LPWORD wPtrNear;
		wBufNear=(LPWORD)calloc((maxObjects+10)*2+2, 1);
		for ( evgPtr=pEvents; evgPtr->evgSize!=0; evgPtr=EVGNEXT(evgPtr) )
		{
			...
			for (evtPtr=EVGFIRSTEVT(evgPtr), n=0; n<evgPtr->evgNCond; evtPtr=EVTNEXT(evtPtr), n++)
			{
				...
				{
					if ((type == OBJ_SPR && bOptimizeForEachActiveObjects) || (type > OBJ_SPR && bOptimizeForEachOtherObjects))
					{
						// Optimization => change condition number
						LPEVP evpPtr = EVTPARAMS(evtPtr);
						int eventCode = forEachLoopIDToConditionCode(type, evpPtr->evp.evpW.evpW0);
						num=-EVTNUM(eventCode);
				...
				if (bOrBefore && evtPtr->evtCode.evtLCode.evtCode != CNDL_ENDCHILDEVENT)
                {
					...
					if (type<0)
					{
						ulPtr=listPointers+*(aListPointers+NUMBEROF_SYSTEMTYPES+type);		// NUMBEROF_SYSTEMTYPES
						(*(ulPtr+num))++;
					}
					else
		...
		sEventPointers=(evtAlways+1)*sizeof(eventOffsets);
		for (uilPtr=listPointers, cpt=sListPointers/4; cpt>0; uilPtr++, cpt--)
		{
			if ( *uilPtr!=0 )
			{
				...
				*uilPtr=sEventPointers;
				sEventPointers+=(ss+1)*sizeof(eventOffsets);
			}
		}
		...
		if(rhPtr->rh4PosOnLoop != nil)
		{
			...
			for (n=0; n < rh4PosOnLoop->Size(); n++)
			{
                ...
                if (rhPtr->rhFrame->numberOfIndexedFastLoops)
                    pLoop->ResetDeltas();
                else
                    delete pLoop;
			}
			...
			for (evtPtr=EVGFIRSTEVT(evgPtr), n=0; n<evgPtr->evgNCond; evtPtr=EVTNEXT(evtPtr), n++)
			{
                ...
                if (bOrBefore && evtPtr->evtCode.evtLCode.evtCode != CNDL_ENDCHILDEVENT)
                {
					...
					if ( (evtPtr->evtFlags&EVFLAGS_ALWAYS)!=0 )
					{
                        ...
                        {
                            eventOffsets* eoPtr = (eventOffsets*)((LPBYTE)eventPointers+evtAlwaysPos);
                            eoPtr->evgOffset = EVGDELTA(evgPtr, pEvents);
                            eoPtr->evtOffset = EVTDELTA(evtPtr, pEvents);
                            evtAlwaysPos+=sizeof(eventOffsets);
                        }
                    }
					...
					if (type<0)
					{
                        ...
                        eventOffsets* eoPtr = (eventOffsets*)((LPBYTE)eventPointers+*lposPtr);
						DWORD delta1 = EVGDELTA(evgPtr, pEvents);
						DWORD delta2 = EVTDELTA(evtPtr, pEvents);
                        eoPtr->evgOffset = delta1;
                        eoPtr->evtOffset = delta2;
						*lposPtr+=sizeof(eventOffsets);
                        ...
                        if (evtPtr->evtCode.evtLCode.evtCode == CNDL_ONLOOP)
						{
							...
							if (evpPtr->evpCode == PARAM_SHORT)
							{
								int fastLoopIndex = evpPtr->evp.evpW.evpW0 - 1;
								if (fastLoopIndex >= 0 && fastLoopIndex < rhPtr->rhFrame->numberOfIndexedFastLoops)
								{
									CCArrayList* rh4PosOnLoop = (CCArrayList*)rhPtr->rh4PosOnLoop;
									CPosOnLoop* posOnLoop = (CPosOnLoop*)rh4PosOnLoop->Get(fastLoopIndex);
...
-(void)evt_BranchSelectedObjects
{
	...
	for (oil = 0; oil < rhPtr->rhMaxOI; oil++)
	{
		...
		if (oilPtr->oilEventCountOR == rh4EventCountOR)
		{
			...
			num = oilPtr->oilObject;
			pHOPrev = nil;
			bool bResetNumOfSelected = true;
			while (num >= 0)
			{
				pHO = rhPtr->rhObjectList[num];
				if (pHO->hoSelectedInOR != 0)
				{
					if (bResetNumOfSelected)
					{
						oilPtr->oilNumOfSelected = 0;
						bResetNumOfSelected = false;
					}
					if (pHOPrev != nil)
					{
    ...
    while ((oil = *(oiOilList+1)) != -1)
    {
        ...
        if ( poil->oilEventCount == rh2EventCount )
        {
            ...
            while(num>=0)
            {
                ...
                if (pHoFound == NULL)
                    break;
                if ( (pHoFound->hoFlags&HOF_DESTROYED)==0 || (rhPtr->rhApp->hdr2Options2 & AH2OPT2_SELECTDELETEDOBJECTSINCHILDEVENTS) != 0)
                ...
                if ( sel.selectedObjects.size() > 0 )
                {
                    ...
                    if ( pHoPrev != NULL )
                    {
                        ...
                        for (int j=1; j<sel.selectedObjects.size(); j++)
                        {
                            ...
                            if (pHo == NULL)
                                break;
                            if ( (pHo->hoFlags & HOF_DESTROYED)==0 || (rhPtr->rhApp->hdr2Options2 & AH2OPT2_SELECTDELETEDOBJECTSINCHILDEVENTS) != 0)
                            {
                                pHoPrev->hoNextSelected = num;
                                poil->oilNumOfSelected++;
                            }
