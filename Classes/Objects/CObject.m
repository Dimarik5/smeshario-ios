...
-(void)generateEvent:(int)code withParam:(int)param
{
	if (hoAdRunHeader->rh2PauseCompteur == 0 && hoAdRunHeader->rhFrame->pTrans == nil)
	{
		...
		LPEVG eventGroup = hoAdRunHeader->rhEvtProg->rhEventGroup;
		LPEXP expToken = hoAdRunHeader->rh4ExpToken;
        BOOL saveActionOn = hoAdRunHeader->rhEvtProg->rh2ActionOn;
        hoAdRunHeader->rhEvtProg->rh2ActionOn = NO;

		code = (-(code + EVENTS_EXTBASE + 1) * 65536);
		code |= (((int) hoType) & 0xFFFF);
		[hoAdRunHeader->rhEvtProg handle_Event:self withCode:code];
		
        hoAdRunHeader->rhEvtProg->rh2ActionOn = saveActionOn;
		hoAdRunHeader->rhEvtProg->rhEventGroup = eventGroup;
		hoAdRunHeader->rh4ExpToken = expToken;
 ...
 */
-(int)checkOrCreateEffectIfNeededByIndex:(int)index withEffectParam:(int)rgba andFillData:(BOOL)fillMode
{
    CRunApp* app = hoAdRunHeader->rhApp;
    if(![app->effectBank isEmpty])
    {
        if(hoEffect != nil && hoEffect->handle == index)
            ...
            if ([hoEffect initializeByIndex:index withEffectParam:rgba]) // Needs a handle (from objects)??
            {
                if(fillMode)
                    [self fillEffectData];
                CLayer* pLayer = app->frame->layers[hoLayer];
                if(pLayer != nil)
                    pLayer->needBackground |= hoEffect->useBackground;
                return [hoEffect getIndexShader];
            } else
                return -1;
        }
...
-(void)fillEffectData
{
    ...
    {
        if(![hoEffect setEffectData:oiPtr->oiEffectData andSize:oiPtr->oiEffectNParams])
            NSLog(@"Values/Parameters does not match, check backdrop shader: %@", [hoEffect getName]);
    }
}
