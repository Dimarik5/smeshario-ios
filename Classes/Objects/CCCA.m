...
-(void)spriteDraw:(CRenderer*)renderer withSprite:(CSprite*)spr andImageBank:(CImageBank*)bank andX:(int)x andY:(int)y
{
	if (subApp!=nil && subApp->run!=nil && subApp->frame != nil)
	{
		...
		if(rtt == nil )
            rtt = [[CRenderToTexture alloc] initWithWidth:fW andHeight:fH andRunApp:hoAdRunHeader->rhApp andSwapMode:YES];

		int rW = rtt->width;
		int rH = rtt->height;

        [rtt bindFrameBuffer];
        renderer->pushScene(0, 0, fW, fH);
        [subApp->run screen_Update];
        renderer->renderAScene(subApp->frame);
        renderer->popScene();
        ...
        else
        {
            ...
            renderer->renderStretch(rtt, x-dx, y-dy, fW, fH, 0, 0, fW, fH);
            //renderer->resetClip();
