	...
	switch (adCta->odDisplayType)
	{
		...
		case 5:	    // CTA_TEXT:
		{
			...
			if(!sameValue)
			{
				...
				int x2 = (int)rc.right;
                ht = [textSurface calculateTextSizeWithString:cachedString andFlags:(short)(dtflags|DT_CALCRECT) andRect:rc andFont:font].height;
	...
	switch (adCta->odDisplayType)
	{
		...
		case CTA_HBAR:
		{
			...
			if (adCta->ocFillType == CTA_FILLTYPE_GRADIENT)
			{
				...
				if (adCta->odDisplayType == CTA_VBAR)	
				{
					...
					renderer->renderGradient(gradient, x, y, cx, cy, effectShader, effect, effectParam);
				}
				else	// CTA_HBAR 
				{
					GradientColor gradient = GradientColor(color1, color2, (hoCommon->ocCounters->ocGradientFlags == CTA_GRAD_HORIZONTAL));
			...
			{
				renderer->renderGradient(GradientColor(color1), x, y, cx, cy, effectShader, effect, effectParam);
			}
			break;
		}	
		case CTA_DIGITS:
		...
		case CTA_TEXT:
		{
			...
			if (hoRect.bottom - hoRect.top != 0)
			{
                ...
                [textSurface draw:renderer withX:hoRect.left andY:hoRect.top andShader:effectShader andEffect:effect andEffectParam:effectParam];
			}
			break;
		}
	}
