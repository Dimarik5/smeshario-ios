...
- (void)checkLayerEffects {
    ...
    for(int n=0; n<nLayers; n++) {
        if((layers[n]->effect & BOP_MASK) == BOP_EFFECTEX) {
            hasLayerEffects = YES;
            break;
        }
    }
    ...
    if(![app->effectBank isEmpty])
    {
        if (effectEx == nil)
        {
            ...
            if ([effectEx initializeByIndex:effectIndex withEffectParam:effectParam])
            {
                if(effectData != nil)
                {
                    if(![effectEx setEffectData:effectData andSize:effectNParams])
                        NSLog(@"Values/Parameters does not match, check frame shader: %@", [effectEx getName]);
                }
...
-(int)checkOrCreateEffectIfNeededByIndex:(int)index
{
    if(![app->effectBank isEmpty])
    {
        if (effectEx == nil) {
            ...
            effectEx = [[CEffectEx alloc] initWithApp:app];
            if ([effectEx initializeByIndex:index withEffectParam:effectParam]) // Needs a handle (from objects)??
            {
                if(![effectEx setEffectData:effectData andSize:effectNParams])
                    NSLog(@"Values/Parameters does not match, check frame shader: %@", [effectEx getName]);
                return [effectEx getIndexShader];
...
//	int dwPSCFlags = 0;
	...
	if (!bSprColBox)
	{
		...
		pSprMask = [app->run->spriteGen getSpriteMask:pSpr withImage:nImg andFlags:GCMF_OBSTACLE andAngle:newAngle andScaleX:newScaleX andScaleY:newScaleY];
		if (pSprMask == nil)
		{
	...
	else
	{
		...
		if (nImg == (unsigned short)-1 || nImg == pSpr->sprImg || (dwSprFlags & SF_OWNERDRAW) != 0)
		{
			sprRc.left = pSpr->sprX1new;		// GetSpriteRect
			sprRc.right = pSpr->sprX2new;
			sprRc.top = pSpr->sprY1new;
		...
		{
			image = [app->imageBank getImageFromHandle:nImg];
			if (image != nil)
			{
...
-(BOOL)bkdCol_TestSprite:(CSprite*)pSpr withImage:(unsigned short)newImg andX:(int)newX andY:(int)newY andAngle:(float)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andFoot:(int)subHt andPlane:(int)nPlane
{
	...
	if (dwLayer == 0)
	{
		...
		{
			// Handle collisions like with the other layers (detect collisions with the objects, not with the collision mask)
			return [self bkdLevObjCol_TestSprite:pSpr withImage:newImg andX:newX andY:newY andAngle:newAngle andScaleX:newScaleX andScaleY:newScaleY andFoot:subHt andPlane:nPlane];
		}
		// Normal mode (no wrap mode, or windowed collision mask)
		else
		{
	...
	return ([app->run->spriteGen spriteCol_TestSprite:pSpr withImage:newImg andX:newX andY:newY andAngle:newAngle andScaleX:newScaleX andScaleY:newScaleY andFoot:subHt andFlags:dwFlags] != nil);
}
...
-(BOOL)colMask_TestSprite:(CSprite*)pSpr withImage:(unsigned short)newImg andX:(int)newX andY:(int)newY andAngle:(float)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andFoot:(int)subHt andPlane:(int)nPlane
{
	...
	{
		// Image sprite not stretched and not rotated, or owner draw sprite?
		pMask = [app->run->spriteGen getSpriteMask:pSpr withImage:nImg andFlags:GCMF_OBSTACLE andAngle:newAngle andScaleX:newScaleX andScaleY:newScaleY];
		if (pMask == nil)
		{
	...
	else
	{
		...
		{
			CImage* pei = [app->imageBank getImageFromHandle:nImg];
			if (pei != nil)
			{
				x1 -= pei->xSpot;
				y1 -= pei->ySpot;
