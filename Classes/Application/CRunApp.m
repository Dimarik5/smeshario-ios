...
-(void)endApplication
{
    ...
    if (joystickGamepad!=nil)
    {
        [joystickGamepad release];
    }
...
//        }
        ...
        renderer->setProjectionMatrix(renderer->topLeft.x, renderer->topLeft.y, renderer->currentRenderState.contentSize.x, renderer->currentRenderState.contentSize.y, renderer->currentRenderState.actProjstate);
    }
    while (false);
...
-(void)endFrame
{
    ...
    if (appRunningState != SL_STARTFRAME || nextFrame != currentFrame)
    {
        ...
        // remove all shaders
        renderer->removeAllShaders();
        // discard all pool texture created in frame.
        renderer->clearPoolCTexture();
        // discard all pool render texture created in frame.
        renderer->clearPoolRenderTexture();
        // discard all pool GL texture created in frame.
        renderer->clearGLPoolTexture();
        // Reset current frame
        currentFrame = -1;
    }
}
...
- (void)createLayerEffects:(CRunFrame *)frame {
    ...
    frame->hasLayerEffects = NO;

    for (int i = 0; i < nLayers; i++) {
        CLayer *pLayer = frame->layers[i];
        if ((pLayer->dwOptions & FLOPT_PREVIOUSEFFECT) != 0) {
            pLayer->dwLayerOptions |= WLF_PREVIOUSEFFECT;
            frame->hasLayerEffects = YES;
        }
        if ((pLayer->effect & EFFECT_MASK) == BOP_EFFECTEX) {
...
-(NSString*)getRelativePath:(NSString*)path
{
    ...
    NSString* pathUpper=[fpath uppercaseString];
    
    //Check if the path is already relative
    if (pathLength < 2 || ([fpath characterAtIndex:1] != ':' && ![fpath hasPrefix:@"//"] && ![fpath hasPrefix:@"/"])) {
        return fpath;
    }
    ...
    if([pathUpper hasPrefix:appEditorPathname])
        return [fpath substringFromIndex:appPathLength+1];
    ...
    if([fpath characterAtIndex:1] == ':' || [fpath hasPrefix:@"//"])
    {
        NSRange searchRange = NSMakeRange(0, [fpath length]);
        NSRange index=[fpath rangeOfString:@"/" options:NSBackwardsSearch range:searchRange];
        if (index.location != NSNotFound)
            return [fpath substringFromIndex:index.location+1];
    }
...
- (NSString *)getPathForWriting:(NSString *)path {
    NSString *normalizedPath = [self getRelativePath:path];
    NSString *lowercasePath = [normalizedPath lowercaseString];

    NSString *basePath = nil;
    NSString *prefixToStrip = nil;

    if ([lowercasePath hasPrefix:@"documents/"]) {
        basePath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject];
        prefixToStrip = @"documents/";
    } else if ([lowercasePath hasPrefix:@"library/"]) {
        basePath = [NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) firstObject];
        prefixToStrip = @"library/";
    } else if ([lowercasePath hasPrefix:@"caches/"]) {
        basePath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject];
        prefixToStrip = @"caches/";
    } else if ([lowercasePath hasPrefix:@"tmp/"] || [lowercasePath hasPrefix:@"temp/"]) {
        basePath = NSTemporaryDirectory();
        prefixToStrip = [lowercasePath hasPrefix:@"tmp/"] ? @"tmp/" : @"temp/";
    } else {
    ...
    if (prefixToStrip != nil) {
        normalizedPath = [normalizedPath substringFromIndex:[prefixToStrip length]];
    }

    NSString *finalPath = [basePath stringByAppendingPathComponent:normalizedPath];
    ...
    BOOL success = [[NSFileManager defaultManager] createDirectoryAtPath:folderPath
                                             withIntermediateDirectories:YES
                                                              attributes:nil
                                                                   error:&error];
    if (!success) {
        NSLog(@"Could not create the directory for writing: %@, error: %@", folderPath, error);
    }
...
{
    // Handle File Provider Storage paths with security scope
    if ([path containsString:@"File Provider Storage"]) {
    ...
    // Check if by any chance file is external one
    ...
    // path is complete??? try to read the info.
    ...
    NSString* fpath = [self getRelativePath:path];
    
    //Check in Documents folder
    NSString* documentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString* documentsPath = [documentsDirectory stringByAppendingPathComponent:fpath];
    if([[NSFileManager defaultManager] fileExistsAtPath:documentsPath])
        return [NSData dataWithContentsOfFile:documentsPath];
    //Cache directory
    NSString* cacheDirectory = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString* cachePath = [cacheDirectory stringByAppendingPathComponent:fpath];
    ...
    if(expansion_available)
    {
        ...
        if (supportDirectory) {
            ...
            NSString* appsupportPath = [appSupportDir stringByAppendingPathComponent:fpath];
    ...
    //App Resources
    NSString* fileName = [fpath stringByDeletingPathExtension];
    ...
    if(embeddedFiles != nil)
    {
        ...
        for(NSUInteger i=0; i<embLen; ++i)
        {
            ...
            if([fpath isEqualToString:embFile->path]
                || [embFile->path containsString:fpath])
                return [embFile open];
        }
    }
    
    if([fpath containsString:@"assets"])
        return [NSData dataWithContentsOfFile:fpath];
...
-(BOOL)resourceFileExists:(NSString*)path
{
    ...
    //App Resources
    NSString* resourcePath = [[NSBundle mainBundle] pathForResource:[fpath stringByDeletingPathExtension] ofType:[fpath pathExtension]];
    if(resourcePath != nil)
        return YES;
