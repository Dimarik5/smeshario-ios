...
-(void)dealloc
{
    ...
    if (eParams!=nil)
    {
        ...
        {
            if(eParams[i]->img_handle != (unsigned short)-1)
...
-(bool)initializeByIndex:(int)index withEffectParam:(int)rgba
{
    @try
    {
        ...
        if(e != nil) {
            ...
            usePixelSize  = (e->options & EFFECTOPT_WANTSPIXELSIZE) != 0 ? YES : NO;
            return [self initializeShader];
        }
    }
    @catch (NSException* ex)
    {
        NSLog(@"Something went wrong with shader %@", name);
    }
    return NO;
}
...
-(bool)initializeShader
{
    ...
    if(indexShader != -1)
    {
        ...
        if(usePixelSize)
            app->renderer->setPixelSizeUse(indexShader);
        return YES;
    }
    return NO;
}
    ...
    if(size == nParams)
    {
        for(int i=0; i < nParams; i++)
        {
            switch(eParams[i]->nValueType)
            {
                ...
                {
                    unsigned short value = (unsigned short)values[i];
...
-(void)setParamAt:(int)paramIdx withTexture:(unsigned short)image_handle
{
    if(eParams != nil)
    {
        if(paramIdx >=0 && paramIdx < nParams)
        {
            ...
            {
                if(eParams[paramIdx]->img_handle != (unsigned short)-1)
                {
        ...
        for(int n=0; n < nParams; n++) {
            switch(eParams[n]->nValueType)
            {
                case EFFECTPARAM_SURFACE:
                {
                    ...
                    if(eParams[n]->img_handle != (unsigned short)-1)
                    {
