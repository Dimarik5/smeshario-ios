...
// Use this definition by uncommenting it to make the primary volume work like the other systems at runtime.
//#define MAINVOLUME_AS_OTHER_RUNTIMES
//#define SILENT_MUTE_SWITCH
...
-(void)audioSessionInterrupted:(NSNotification*)notification
{
    ...
    else if (type == AVAudioSessionInterruptionTypeEnded)
    {
        ...
        {
            // Attempt to resume the audio session
            [self requestAudioSession];
            NSLog(@"Audio interruption ended, requesting audio session ...");
        }
...
- (void)audioSessionSilenceHint:(NSNotification *)notification {
    NSDictionary *info = notification.userInfo;
    AVAudioSessionSilenceSecondaryAudioHintType hintType = (AVAudioSessionSilenceSecondaryAudioHintType)[info[AVAudioSessionSilenceSecondaryAudioHintTypeKey] unsignedIntegerValue];

    if (hintType == AVAudioSessionSilenceSecondaryAudioHintTypeBegin) {
        NSLog(@"Received silence hint: Begin — we should lower or pause audio.");
        flopAtEnd = YES;
    } else if (hintType == AVAudioSessionSilenceSecondaryAudioHintTypeEnd) {
        NSLog(@"Silence hint ended — we can resume audio.");
        if(flopAtEnd)
        {
            [self requestAudioSession];
            flopAtEnd = NO;
...
-(id)initWithApp:(CRunApp*)app
{
    if(self = [super init])
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(audioSessionSilenceHint:)
...
#ifdef MAINVOLUME_AS_OTHER_RUNTIMES
                    volumes[channel] = 100;
#else
...
#ifdef MAINVOLUME_AS_OTHER_RUNTIMES
        [channels[channel] setVolume:(mainVolume*volumes[channel])/100];
#else
...
-(void)setVolumeSample:(short)handle withVolume:(int)v
{
    ...
    for (n = 0; n < NCHANNELS; n++)
    {
        ...
        if (channels[n] != nil)
        {
            ...
            {
                volumes[n]=v;
                [channels[n] setVolume:(mainVolume*v)/100];
            }
        }
#else
        if (channels[n] != nil)
...
-(void)setVolumeChannel:(int)channel withVolume:(int)v
{
    ...
    if (channel >= 0 && channel < NCHANNELS)
    {
        ...
        if (channels[channel] != nil)
        {
            [channels[channel] setVolume:(mainVolume*v)/100];
        }
#else
        volumes[channel]=v;
        if (channels[channel] != nil)
...
-(void)setMainVolume:(int)v
{
    ...
    for (n=0; n<NCHANNELS; n++)
    {
        ...
        if (channels[n]!=nil)
        {
            [channels[n] setVolume:(mainVolume*volumes[n])/100];
        }
#else
        volumes[n]=v;
        if (channels[n]!=nil)
...
-(BOOL)requestAudioSession
{
...
#endif
    AVAudioSessionCategoryOptions opts = AVAudioSessionCategoryOptionDuckOthers
    ...
    [session setCategory:category withOptions:opts error:&error];
    BOOL bResult = [session setActive:YES withOptions:AVAudioSessionSetActiveOptionNotifyOthersOnDeactivation error:&error];

    if (error) {
        NSLog(@"Forced reactivation failed: %@", error.localizedDescription);
    } else {
        NSLog(@"Audio session normally reactivated after app active");
    }
