...
public:
    ...
    std::vector<RenderingState> renderStateStack;
    std::vector<Mat3f> transformStack;
    ...
    std::vector<CShader*> shaderStack;
    CShadersVector shaders;
    int iShader;

    SceneRender renderScene;

    std::vector<SceneRender> sceneStack;
    ...
    void beginLayer(int inkEffect, bool needBackground);
