...
-(CMask*)getSpriteMask:(CSprite*)ptSpr withImage:(unsigned short)newImg andFlags:(int)nFlags andAngle:(float)newAngle andScaleX:(double)newScaleX andScaleY:(double)newScaleY
{
    if (ptSpr != nil)
    {
        ...
        {
            unsigned short nImg = newImg;
            if (nImg == (unsigned short)-1)
            {
                nImg = ptSpr->sprImg;
            }
            if (nImg != (unsigned short)-1)
...
-(void)pasteSpriteEffect:(CRenderer*)renderer withImage:(unsigned short)iNum andX:(int)iX andY:(int)iY andFlags:(int)flags andInkEffect:(int)effect andInkEffectParam:(int)effectParam andEffectShader:(int)effectShader
{
    ...
    // Blit
    renderer->renderImage(ptei, x1, y1, ptei->width, ptei->height, effectShader, effect, effectParam);
...
//    uint64_t start = mach_absolute_time();
    ...
    int layerShader = -1;
    int layerEffect = 0;
    int layerEffectParam = 0;
    //BOOL useResampling = (app->hdr2Options & AH2OPT_ANTIALIASED) != 0;
    ...
    for (ptSpr = ptFirstSpr; ptSpr != nil; ptSpr = ptSpr->objNext)
    {
        ...
        if (currentLayer != (ptSpr->sprLayer & 0xFFFE)) {
            int nLayer = ptSpr->sprLayer / 2;
            currentLayer = nLayer * 2;

            CLayer *layer = nil;
            BOOL bIgnoreLayerEffect = false;
            bSkipLayer = false;
            if (nLayer < frame->nLayers) {
                layer = frame->layers[ptSpr->sprLayer / 2];
                renderer->setCurrentLayer(layer);
                updateLayer = true;
                bIgnoreLayerEffect = ((layer->dwLayerOptions & WLF_PREVIOUSEFFECT) != 0);
            }

            if (!bIgnoreLayerEffect) {
                if (pendingLayer) {
                    renderer->layerToFrame(layerShader, layerEffect, layerEffectParam);
                    pendingLayer = false;
                }
                layerEffect = BOP_COPY;
                if (layer != nil) {
                    int dwRGBA = 0xFFFFFFFF;
                    // Check for BOP_BLEND first to potentially skip further checks
                    if (layer->effect == BOP_BLEND) {
                        if (layer->effectParam == 128) {
                            bSkipLayer = true;
                        }
                    } else {
                        if ((layer->effect & BOP_MASK) == BOP_EFFECTEX) {
                            if (layer->effectEx != nil)
                                dwRGBA = [layer->effectEx getRGBA];
                        } else if ((layer->effect & BOP_RGBAFILTER) != 0)
                            dwRGBA = layer->effectParam;
                        // Skip if the alpha channel is zero
                        if ((dwRGBA & 0xFF000000) == 0) {
                            bSkipLayer = true;
                        }
                    }
                    if (layer->hasTransformation || (!bSkipLayer &&
                            (layer->effect != BOP_COPY
                                    && (layer->effect != BOP_RGBAFILTER || dwRGBA != 0xFFFFFFFF)))) {
                        renderer->beginLayer(layer->effect, layer->needBackground);
                        layerEffect = layer->effect;
                        layerEffectParam = dwRGBA;
                        layerShader = layer->effectShader;
                        pendingLayer = true;
                    }
                }
            }
        ...
        if(updateLayer)
        {
            renderer->setLayerTransform();
            updateLayer = false;
        }

//        NSString* name = ptSpr->sprExtraInfo != nil ? ptSpr->sprExtraInfo->hoOiList->oilName
//                            : (ptSpr->sprRout != nil ? NSStringFromClass(object_getClass(ptSpr->sprRout)) :  @"backdrop");
        // Sprite ownerdraw
        if ((ptSpr->sprFlags & SF_OWNERDRAW) != 0 && ptSpr->sprRout != nil)
        {
//            NSLog(@"Ownerdraw sprite %@ located at: %d, %d ", name, ptSpr->sprX1, ptSpr->sprY1);
        ...
        {
//            NSLog(@"Normal sprite %@ located at: %d, %d ", name, ptSpr->sprX1, ptSpr->sprY1);
            [ptSpr draw:renderer];
        }
        ptSpr->sprFlags &= ~(SF_REAF | SF_REAFINT);
    }
    
    // Done with sprite? Render any pending layer
    if(pendingLayer) {
        renderer->layerToFrame(layerShader, layerEffect, layerEffectParam);
    }
        ...
        if (bEvenNoCol || (ptSpr->sprFlags & SF_RAMBO) != 0)
        {
            ...
            */
            if (lx >= ptSpr->sprX1 && lx < ptSpr->sprX2 && ly >= ptSpr->sprY1 && ly < ptSpr->sprY2)
            {
                ...
                CMask* pMask = [self getSpriteMask:ptSpr withImage:(unsigned short)-1 andFlags:nGetColMaskFlag];
                if(pMask != nil)
                {
