...
-(BOOL)animIn:(int)vbl
{
	...
	if (raAnimDir != dir)
	{
		...
		if (raAnimDirOffset != adPtr)
		{
            ...
            unsigned short frame = adPtr->adFrames[raAnimFrame];
			if (raAnimStopped == NO)
			{
				hoPtr->roc->rcImage = frame;
				if (frame==(unsigned short)-1)
				{
			...
			if (frame >= raAnimNumberOfFrame)
			{
				...
				if (raAnimRepeat != 0)					//; On boucle?
				{
					...
					if (raAnimRepeat == 0)
					{
						...
						{
                            unsigned short image = adPtr->adFrames[raAnimFrame];
