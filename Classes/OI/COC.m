...
-(void)fillEffectData:(COI*)oiPtr
{
    if(oiPtr->oiEffectOffset != 0 && ocEffect != nil)
    {
        ...
        {
            if(![ocEffect setEffectData:oiPtr->oiEffectData andSize:oiPtr->oiEffectNParams])
               NSLog(@"Values/Parameters does not match, check backdrop shader: %@", [ocEffect getName]);
        }
    }
}
