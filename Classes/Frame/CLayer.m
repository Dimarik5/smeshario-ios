...
-(int)checkOrCreateEffectIfNeededByIndex:(int)index
{
    ...
    if(![app->effectBank isEmpty])
    {
        if (effectEx == nil) {
            ...
            if ([effectEx initializeByIndex:index withEffectParam:effectParam])
            {
                if(![effectEx setEffectData:effectData andSize:effectNParams])
                    NSLog(@"Values/Parameters does not match, check layer shader: %@", [effectEx getName]);
...
-(int)checkOrCreateEffectIfNeededByName:(NSString*)name
{
    CRunApp* app = frame->app;
    if(![app->effectBank isEmpty])
    {
...
-(int)checkOrCreateEffectIfNeeded:(CRunApp*)app
{
    if(![app->effectBank isEmpty])
    {
        if (effectEx == nil) {
            ...
            effectEx = [[CEffectEx alloc] initWithApp:app];
            if ([effectEx initializeByIndex:effectIndex withEffectParam:effectParam]) // Needs a handle (from objects)??
            {
                if(![effectEx setEffectData:effectData andSize:effectNParams])
                    NSLog(@"Values/Parameters does not match, check layer shader: %@", [effectEx getName]);
