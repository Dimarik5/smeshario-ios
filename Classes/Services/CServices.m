...
int SemiTranspToAlpha(int semi)
{
    return (semi == 128 ? 0: (255-semi*2));
}

int AlphaToSemiTransp(int alpha)
{
...
+ (NSMutableParagraphStyle *)cachedParagraphStyle:(short)flags {
    NSMutableParagraphStyle* paragraphStyle = [[NSParagraphStyle defaultParagraphStyle] mutableCopy];
    paragraphStyle.lineBreakMode = NSLineBreakByWordWrapping;  // Default mode
   if((flags & DT_SINGLELINE) != 0)
   {
       if((flags & DT_ENDELLIPSIS) != 0)
           paragraphStyle.lineBreakMode = NSLineBreakByTruncatingTail;
    ...
    //Horizontal alignment
    NSTextAlignment hAlignment = NSTextAlignmentLeft;
    if (flags & 0x0001)
...
+(int)drawText:(CBitmap*)bitmap withString:(NSString*)s andFlags:(short)flags andRect:(CRect)rc andColor:(int)rgb andFont:(CFont*)font andParagraghStyle:(NSMutableParagraphStyle *)paragraphStyle andAntialias:(BOOL)smoothfonts
{
    //PROFILE_BEGIN();
    // Empty string?
    if ([s length]==0)
    ...
    // Assume UIFont and NSMutableParagraphStyle are cached properties of this class or provided externally for reuse
    UIFont *f = [font createFont];  // Consider moving font creation outside this method if the font doesn't change often
    ...
    // Simplified text measurement and drawing block
    CGSize size = [@" " sizeWithAttributes:@{NSFontAttributeName: f}];  // Assuming modern APIs only
    CGSize rectSize = [s boundingRectWithSize:CGSizeMake(rc.width(), CGFLOAT_MAX)
                                     options:NSStringDrawingUsesLineFragmentOrigin
                                  attributes:@{NSFontAttributeName: f, NSParagraphStyleAttributeName: paragraphStyle}
                                     context:nil].size;
    int hLine=size.height;
    ...
    // Calculating vertical position based on flags
    CGFloat y = rc.top;
    if (flags & DT_VCENTER) {
        y = (rc.bottom + rc.top) / 2 - rectSize.height / 2;
    ...
    UIColor* color = [UIColor colorWithRed:getR(rgb)/255.0f green:getG(rgb)/255.0f blue:getB(rgb)/255.0f alpha:1];
    // Draw the text only if a bitmap context is available
    if (context) {
        CGContextSetShouldAntialias(context, smoothfonts);
        CGContextSetShouldSmoothFonts(context, smoothfonts);

        CGRect textRect = CGRectMake(rc.left, y, rc.width(), MAX(rc.height(), rectSize.height));
        [s drawWithRect:textRect
                 options:NSStringDrawingUsesLineFragmentOrigin
             attributes:@{NSFontAttributeName: f, NSParagraphStyleAttributeName: paragraphStyle, NSForegroundColorAttributeName: color}
                 context:nil];
    ...
    //PROFILE_END(@"drawing text");
    // Return the new bottom position of the drawn text
    return y + rectSize.height;
