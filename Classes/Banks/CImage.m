...
-(id)initWithApp:(CRunApp*)a
{
	...
	opaqueMask = NO;
	wrapS = wrapT = GL_CLAMP_TO_EDGE;
    premultimage = (app->hdr2Options2 & AH2OPT2_PREMULTIPLIEDIMAGES) != 0;
	return self;
}
...
{
	file=f;
	handle = [file readAUnsignedShort];
	[file skipBytes:16];
...
+ (CImage*)loadUIImage:(UIImage*)image
{
	...
	newImage->useCount = 1;
	newImage->handle = (unsigned short)-1;
