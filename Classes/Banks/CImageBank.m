...
{
	runApp=app;
    ispremultiply = (app->hdr2Options2 & AH2OPT2_PREMULTIPLIEDIMAGES) != 0;
	return self;
}
-(void)dealloc
{
...
-(CImage*)getImageFromHandle:(unsigned short)handle
{
	if (handle>=0 && handle<nHandlesTotal)
	    if (handleToIndex[handle]!=(unsigned short)-1)
...
{
    unsigned short us = num;
    if(us == (unsigned short)-1 || us > nHandlesReel)
        return (short)-1;
...
-(void)load
{
	...
	for (h=0; h<nHandlesReel; h++)
	{
	    ...
	    if (objUseCount!=0)
	    {
			if (images!=nil && handleToIndex[h]!=(unsigned short)-1 && images[handleToIndex[h]]!=nil)
			{
	...
	handleToIndex=(unsigned short*)malloc(nHandlesReel*sizeof(unsigned short));
	for (n=0; n<nHandlesReel; n++)
	{
	    handleToIndex[n]=(unsigned short)-1;
	}
	for (n=0; n<nImages; n++)
	{
	    handleToIndex[images[n]->handle]=(unsigned short)n;
	}
	nHandlesTotal=nHandlesReel;
        ...
        if (img->useCount<=0)
        {
            ...
            for (n=0; n<nImages; n++)
            {
                if (images[n]==img)
                {
                    [images[n] release];
                    images[n]=nil;
                    handleToIndex[handle]=(unsigned short)-1;
                    break;
                }
            }
        }
...
-(void)loadUnloaded
{
    ...
    for (h=0; h<nHandlesReel; h++)
    {
        ...
        {
            if(handleToIndex[h] == (unsigned short)-1 || images[handleToIndex[h]] == nil)
            {
...
-(void)unloadLoaded
{
    ...
    for (h=0; h<nHandlesReel; h++)
    {
        ...
        if (discardCount > 0)
        {
            ...
            useCountCopy[h] = useCount[h];
            if (useCount[h] == 0 && images!=nil && handleToIndex[h]!=(unsigned short)-1 && images[handleToIndex[h]]!=nil)
            {
                //NSLog(@"Discarded handle:%i", h);
                runApp->renderer->removeTexture(images[handleToIndex[h]], NO);
#ifdef TEXTURE_RAM_GC
                runApp->renderer->cleanUnused();
#endif
                [images[handleToIndex[h]] release];
                images[handleToIndex[h]]=nil;
                handleToIndex[h]=(unsigned short)-1;
            }
            count++;
        }
    }
...
-(unsigned short)addImage:(CImage*)img withXSpot:(short)xSpot andYSpot:(short)ySpot andXAP:(short)xAP andYAP:(short)yAP andCount:(short)count
{
	...
	{
	    if (handleToIndex[h]==(unsigned short)-1)
	    {
			hFound=h;
			break;
	    }		
	...
	if (hFound==-1)
	{
        unsigned short* newHToI=(unsigned short*)malloc((nHandlesTotal+10)*sizeof(unsigned short));
	...
	// Ajoute la nouvelle image
	handleToIndex[hFound]=(unsigned short)iFound;
	images[iFound]=[[CImage alloc] initWithApp:runApp];
		...
		if (handles[h]>=0 && handles[h]<nHandlesTotal)
		{
			if (offsetsToImages[handles[h]]!=0)
			{
				...
				{	
					...
					// Ajoute la nouvelle image
					handleToIndex[handles[h]]=(unsigned short)iFound;
					images[iFound]=[[CImage alloc] initWithApp:runApp];
...
-(void)loadImageByHandle:(unsigned short)handle
{
    if (offsetsToImages[handle]!=0)
    {
        if ([self getImageFromHandle:handle]==nil)
        {
            ...
            // Ajoute la nouvelle image
            handleToIndex[handle]=(unsigned short)iFound;
            images[iFound]=[[CImage alloc] initWithApp:runApp];
            images[iFound]->useCount=1;
            [runApp->file seek:offsetsToImages[handle]];
            [images[iFound] load:runApp->file];
            images[iFound]->bPrunable = false;
        }
        else
        {
