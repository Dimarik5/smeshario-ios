...
-(void)dealloc
{
    if (extensions!=nil)
    {
        for (int n=0; n<extMaxHandles; n++)
        {
            if(extensions[n] != nil)
                [extensions[n] release];
        }
        free(extensions);
    }
...
-(void)loadList 
{
	...
	if (extCount == 0 && extMaxHandles != 0)
	{
        extensions[2] = [[CExtLoad alloc] initWithHandle:2 andName:@"kcfile" andSubtype:@""];
        extensions[1] = [[CExtLoad alloc] initWithHandle:1 andName:@"kcini" andSubtype:@""];
        // INSERTPOS
		// extensions[xx] = [[CExtLoad alloc] initWithHandle:hh andName:@"" andSubtype:@""];
		for (n=0; n<extMaxHandles; n++)
		{
			CExtLoad* e = extensions[n];
			if (e != nil)
			{
