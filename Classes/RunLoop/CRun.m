...
-(int)allocRunHeader
{
	...
	cndGroupAddress = new std::vector<eventGroup*>;
    
	rh4PosOnLoop = nil;
	rh4CurrentFastLoop = nil;
	rh4CurrentFastLoopIndex = -1;
    rh4ComplexOnLoop = NO;
	if (rhFrame->numberOfIndexedFastLoops != 0)
	{
		rh4PosOnLoop = new CCArrayList();
		rh4FastLoops = [[CArrayList alloc] init];
		for (int i=0; i<rhFrame->numberOfIndexedFastLoops; i++)
		{
			CPosOnLoop* posOnLoop = new CPosOnLoop(nil, i);
			rh4PosOnLoop->Add(posOnLoop);
			
			CLoop* pLoop=[[CLoop alloc] init];
			pLoop->name=nil;
			pLoop->flags=0;
			[rh4FastLoops add:pLoop];
		}
...
-(void)f_KillObject:(int)nObject withFast:(BOOL)bFast
{
    ...
    if ( poil != nil && poil->oilObject == (short)0x8000 )        // last object instance?
    {
        ...
        if(hoPtr->hoEffect != nil && hoPtr->ros->rsEffectShader != -1)
        {
...
-(void)pausePlay:(int)mode
{
    ...
    if (rh2PauseCompteur == 1)
    {
        ...
        // ------------------------------
        if(mode == 1)
            [rhApp->soundPlayer pause:3];
    }
}
/** Quits the pause mode.
 */
-(void)resumePlay:(int)mode
{
    // Uniquement au dernier retour
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if (rh2PauseCompteur>0)
...
-(void)screen_Update
{
    if (rhApp->frame != nil)
    {
        rhApp->renderer->beginFrame(rhApp);
    }
    ...
    {
        rhApp->renderer->renderToScreen((rhApp->hdr2Options & AH2OPT_ANTIALIASED) != 0, rhApp->gaBorderColour, rhFrame->effectShader, rhFrame->effect, rhFrame->effectParam);
    }
}
    ...
    if (pOiColList!=nil)
    {
        for (; *pOiColList!=-1; pOiColList+=2)
        {
            ...
            while(nObject>=0)
            {
                ...
                if ((pHox->hoFlags&HOF_NOCOLLISION)==0 && (nLayer<0 || (nLayer>=0 && nLayer==pHox->hoLayer)))
                {
...
/*
-(CArrayList*)objectAllCol_IXY:(CObject*)pHo withImage:(unsigned short) newImg andAngle:(int)newAngle andScaleX:(float)newScaleX andScaleY:(float)newScaleY andX:(int)newX andY:(int)newY
{
	CArrayList* list = nil;
