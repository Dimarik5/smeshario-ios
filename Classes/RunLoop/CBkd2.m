...
-(void)fillEffectData:(COI*)oiPtr
 {
     if(oiPtr->oiEffectOffset != 0 && bkdEffect != nil)
     {
         ...
         {
             if(![bkdEffect setEffectData:oiPtr->oiEffectData andSize:oiPtr->oiEffectNParams])
                 NSLog(@"Values/Parameters does not match, check background shader: %@", [bkdEffect getName]);
         }
     }
 }
