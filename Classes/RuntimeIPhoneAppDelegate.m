...
-(void)applicationDidBecomeActive:(UIApplication *)application
{
    ...
    [runApp->soundPlayer requestAudioSession];
    ...
    if (runApp->run!=nil)
    {
        ...
        {
            NSLog(@"resuming ...");
            if(runApp->run->rh2PauseCompteur > 0) // a loop of pause/resume?
                [runApp->run resume2];
            else
                [runApp->soundPlayer resume:2]; // no pause in object only pending resume sound
        }
